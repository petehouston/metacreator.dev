...
| Tool runners | 🟡 | **81 registered**, each with a catalog row (`ToolServiceProvider::RUNNERS`; the drift test asserts both directions) — every free tool, plus one account tool. **79 are published and visible**; two are seeded as drafts because their upstream stopped serving what they need to a datacentre IP, and the reasoning is in [07](07-tool-catalog.md#built-not-published). What remains of [07](07-tool-catalog.md) is the account and premium tiers, which need providers rather than runners |
